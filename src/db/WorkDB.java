...
public class WorkDB {
	...
	public static void create(String url, String nameDB, String login, String password) {
		...
		db.update("CREATE TABLE performer ( id_performer INT AUTO_INCREMENT, name VARCHAR(50) NOT NULL UNIQUE,"
				+ "password VARCHAR(50) NOT NULL, k FLOAT(5,3) NOT NULL, PRIMARY KEY(id_performer))");

		DaoPerformer dp = new DaoPerformer(db);
		dp.insert(new Performer("Ravshan & Dzamshut", "pass", 1.500));
		dp.insert(new Performer("Ahmed & Magomed", "pass", 1.000));
		...
		db.update("CREATE TABLE orders ( id_order INT AUTO_INCREMENT, id_client INT NOT NULL,"
				+ "id_performer INT, square INT NOT NULL, floor INT NOT NULL, k FLOAT(5,3) NOT NULL,"
		...
		dr.insert(new Orders(1, 3, 1690, 9, 1.900, 1));
		dr.insert(new Orders(2, 1, 1500, 5, 0.674, 1));
